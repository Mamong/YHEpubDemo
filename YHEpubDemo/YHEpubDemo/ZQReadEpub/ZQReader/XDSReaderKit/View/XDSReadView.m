...
- (void)linkPushed:(DTLinkButton *)button{
    ...
    if (noteModel) {
//        [XDSReaderUtil showAlertWithTitle:@"笔记内容" message:noteModel.note];
        
        
        //创建AlertController对象 preferredStyle可以设置是AlertView样式或者ActionSheet样式
        UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"笔记内容" message:noteModel.note preferredStyle:UIAlertControllerStyleAlert];
        //创建取消按钮
        ...
        UIAlertAction *action2 = [UIAlertAction actionWithTitle:@"删除" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            ...
            [[XDSReadManager sharedManager] delNoteModel:noteModel];
            [self delLineForNote:noteModel];
//            [self setNeedsLayout];
            [self reloadView];
        }];
        //添加文本框
//        [alertC addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
//            textField.text = [self.heroArr[indexPath.row] name];
//        }];
        //添加按钮
        ...
        for (UIView* next = [self superview]; next; next = next.superview) {
            ...
            if ([nextResponder isKindOfClass:[UIViewController class]]) {
                [(UIViewController *)nextResponder presentViewController:alertC animated:YES completion:nil];
                break;
            }
        }
...
- (void)showPhotoBrowserWithImage:(NSString *)imageurl {
    ...
    _imagePath = [imageurl stringByReplacingOccurrencesOfString:imageName withString:@""];
    ...
    browser.currentPhotoIndex = [imageSrcArray containsObject:imageName]?[imageSrcArray indexOfObject:imageName]:0; // 弹出相册时显示第几张图片
...
- (id <MWPhoto>)photoBrowser:(MWPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index {
    ...
    if (index < imageSrcArray.count) {
        ...
        NSString *imageUrl = [_imagePath stringByAppendingString:scr];
        MWPhoto *photo = [MWPhoto photoWithURL:[NSURL fileURLWithPath:imageUrl]];
        return photo;
    }
    return nil;
}
...
//删除笔记
- (void)delLineForNote:(XDSNoteModel *)noteModel
{
    _selectRange = [_content rangeOfString:noteModel.content];
    [_readAttributedContent removeAttribute:NSUnderlineStyleAttributeName range:_selectRange];
    [_readAttributedContent removeAttribute:NSUnderlineColorAttributeName range:_selectRange];
    [_readAttributedContent removeAttribute:NSLinkAttributeName range:_selectRange];
